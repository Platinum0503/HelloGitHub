...
void luuFile(sach s[], int n){
    ...
    fprintf(f, "%d\n", n);
    for(int i = 0; i < n;i++){
        fprintf(f, "%d|%s|%s|%d", s[i].id, s[i].ten, s[i].tenTacGia, s[i].namXuatBan);
    }
    fclose(f);
    printf(">>>Da luu file!\n");
}
int docFile(sach **s){
    FILE *f = fopen("quanLySach.txt", "r");
    if(f == NULL){
    ...
    *s = (sach*)malloc(n * sizeof(sach));
    ...
    for(int i = 0;i < n;i++){
        fscanf(f, "%d|%s|%s|%d", &(*s)[i].id, (*s)[i].ten, (*s)[i].tenTacGia, &(*s)[i].namXuatBan);
    }
    fclose(f);
