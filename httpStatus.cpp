...
enum class StatusCategory {SUCCESS, REDIRECT, CLIENT_ERROR, SERVER_ERROR};
...
bool isSuccess(HttpResponse response){
    //check status có thuộc SUCCESS ko
...
bool isError(HttpResponse response){
    ...
    return cat == StatusCategory::CLIENT_ERROR ||
...
void printResponse(HttpResponse response){
    ...
    cout << "Category:      " << getCategoryName(getCategory(response.status)) << endl;
    cout << "Message:       " << getStatusMessages(response.status) << endl;
    cout << "Body:          " << response.body << endl;
    cout << "headers:       " << response.headers << endl;    
...
int main(){
    ...
    response2.headers = "Contents-Type: Text/html";
    ...
    response3.headers = "Contents-Type: text/plain";
    ...
    response4.headers = "Location: /users/123";
